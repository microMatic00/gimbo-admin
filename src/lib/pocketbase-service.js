...
export class PocketBaseService {
    ...
    try {
      return await pb
        ...
        .getList(options.page || 1, options.perPage || 50, {
          ...
          $autoCancel:
            options.$autoCancel !== undefined ? options.$autoCancel : false,
        });
    } catch (error) {
      // Solo registrar el error si no es por una cancelación intencional
      if (error.name !== "AbortError") {
        console.error(
          `Error al obtener registros de ${this.collection}:`,
          error
        );
      }
      throw error;
  ...
  async getById(id, options = {}) {
    ...
    try {
      ...
      if (error.name !== "AbortError") {
        console.error(
          `Error al obtener registro ${id} de ${this.collection}:`,
          error
        );
      }
      throw error;
  ...
  async create(data, options = {}) {
    ...
    try {
      ...
      if (error.name !== "AbortError") {
        // Si PocketBase devuelve detalles de validación, muéstralos
        if (error?.data) {
          try {
            console.error(
              `Error al crear registro en ${this.collection}:`,
              error.message,
  ...
  async update(id, data, options = {}) {
    ...
    try {
      ...
      return await pb
        .collection(this.collection)
        .update(id, data, mergedOptions);
    } catch (error) {
      if (error.name !== "AbortError") {
        console.error(
          `Error al actualizar registro ${id} en ${this.collection}:`,
          error
        );
      }
      throw error;
  ...
  async delete(id, options = {}) {
    ...
    try {
      ...
      if (error.name !== "AbortError") {
        console.error(
          `Error al eliminar registro ${id} de ${this.collection}:`,
          error
        );
      }
      throw error;
